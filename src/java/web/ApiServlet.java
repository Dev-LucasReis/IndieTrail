...
public class ApiServlet extends HttpServlet {
  ...
  private void processGame(JSONObject file, HttpServletRequest request, HttpServletResponse response) throws Exception {
    if (request.getSession().getAttribute("user") == null) {
        response.sendError(401, "Unauthorized: no session");
        file.put("error", "Unauthorized: no session");
    } else if (request.getMethod().toLowerCase().equals("get")) {
        if (request.getParameter("history") != null) {
            file.put("list", new JSONArray(GamesIndie.getHistoryList()));
        } else {
            file.put("list", new JSONArray(GamesIndie.getList()));
        }
    }else if (request.getMethod().toLowerCase().equals("post")){
        JSONObject body = getJSONBody(request.getReader());
        String gameTitle = body.getString("gameTitle");
        String gameGenre = body.getString("gameGenre");
        String gamePlatform = body.getString("gamePlatform");
        String gameDeveloper = body.getString("gameDeveloper");
        String gamePublisher = body.getString("gamePublisher");
        Date releaseDate = Date.valueOf(body.getString("releaseDate"));
        double gamePrice = body.getDouble("gamePrice");
        GamesIndie.addGamesIndie(gameTitle, gameGenre, gamePlatform, gameDeveloper, gamePublisher, releaseDate, gamePrice);
    } else {
        response.sendError(405, "Method not allowed");
        file.put("error", "Method not allowed");
    }
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            } else if (request.getRequestURI().endsWith("/api/games")) {
                processGame(file, request, response);
            } else {
